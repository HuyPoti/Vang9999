...
export class OrderService {
  ...
  async create(createOrderDto: CreateOrderDto) {
    this.logger.log(
      `Incoming order request for customer: ${createOrderDto.customer_name}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Preparing to send confirmation email to: ${savedOrder.email}`,
      );
      this.mailService
        ...
        .catch((e) =>
          this.logger.error(`Error sending confirmation email: ${e.message}`),
        );
  ...
  async exportToExcel(
    res: any,
    query?: { search?: string; status?: OrderStatus },
  ) {
  ...
  async updateStatus(id: string, status: OrderStatus) {
    ...
    if (order.status === OrderStatus.PENDING) {
      ...
      if (![OrderStatus.SHIPPING, OrderStatus.CANCELLED].includes(status)) {
        throw new Error(
          'Đơn hàng đã xác nhận cần được Chuyển hàng trước khi Hoàn thành',
        );
      }
   ...
   */
  async getProductStatistics(dateRange?: {
    startDate?: string;
    endDate?: string;
  }) {
  ...
  async exportStatistics(
    ...
    overviewSheet.addRow({
      metric: '  - Chờ xác nhận',
      value: orderStats.status_breakdown.pending,
    });
    overviewSheet.addRow({
      metric: '  - Đã xác nhận',
      value: orderStats.status_breakdown.confirmed,
    });
    overviewSheet.addRow({
      metric: '  - Đang giao',
      value: orderStats.status_breakdown.shipping,
    });
    overviewSheet.addRow({
      metric: '  - Hoàn thành',
      value: orderStats.status_breakdown.completed,
    });
    overviewSheet.addRow({
      metric: '  - Đã hủy',
      value: orderStats.status_breakdown.cancelled,
    });
